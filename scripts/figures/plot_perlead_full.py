...
for k, name in enumerate(te):
    ...
    ws = np.stack([af[st:st+W] for st in range(0, len(af)-W, H)])
    raw = np.stack([a[st:st+W, 0] for st in range(0, len(a)-W, H)])
...
pd.DataFrame(out).to_csv(f'{ROOT}/outputs/perlead_full.csv', index=False)
...
ax.text(47.5, 1.2, 'persistence parity', fontsize=5.2, color='#777777', ha='right')
...
fig.tight_layout()
save_pub(fig, f'{ROOT}/outputs/perlead_full')
print(f'wrote outputs/perlead_full: full@1h {skill[0]:+.0f}% @48h {skill[47]:+.0f}% | p999@8h {sk[0.999][7]:+.0f}%')
